...
restart = () => {
    ...
    isPaused = false;
    restart = () => {
		location.reload(); // Refresh the browser tab
	}
	
    // Clear existing game entities
    bomb = null;
    missilePool.onCanvas.forEach(missile => {
        missilePool.ready.push(missile);
    });
    missilePool.onCanvas.length = 0;

    alienPools.onCanvas.forEach(alien => {
        alienPools[alien.type].push(alien);
    });
    alienPools.onCanvas.length = 0;

    explosionPool.onCanvas.forEach(explosion => {
        explosionPool.queue.push(explosion);
    });
    explosionPool.onCanvas.length = 0;

    // Reset wave data
    waveData.types.forEach(t => { 
        alienPools.onCanvasCount[t] = 0;
    });
    waveData.curWave = 0; // Start from the first wave
    waveData.leftInCurWave = Object.assign({}, waveData.waves[0].aliens);

    // Clear key events and reset powerup
    clearKeyEvents();
    powerupPool.active = -1;
    powerupPool.time = powerupPool.timeout;
    waveData.newWave = true;
    ...
    init(); // Call the init function to start the game from the beginning
}
...
init = () => {
    ...
    display = {
        ...
        restartButton: document.getElementById('restart-btn'), // Assuming you have this button in your HTML
        pauseButton: document.getElementById('pause-btn') // Assuming you have this button in your HTML
		
    };
    
    display.restartButton.addEventListener('click', restart);
    display.pauseButton.addEventListener('click', pauseGame);
